...
const mostBlogs = blogs => {
  ...
  const maxAuthor = _.maxBy(_.keys(authorBlogs), o => authorBlogs[o])
  return { author: maxAuthor, blogs: authorBlogs[maxAuthor] }
}

const mostLikes = blogs => {
  const authorWork = _.groupBy(blogs, 'author')
  const authorLikes =  _.mapValues(authorWork, workList =>
    _.reduce(workList, (sum, n) => sum + n.likes, 0))
